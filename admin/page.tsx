...
export default function FactoryGlossaryAdminPage() {
  ...
  async function refresh() {
    setMsg("Loading...");
    try {
      const r = await fetch("/api/factory-glossary?force=true", { cache: "no-store" });
  ...
  async function adminPost(payload: any) {
    const r = await fetch("/api/admin/factory-glossary", {
      method: "POST",
  ...
  const filtered = useMemo(() => {
    ...
    if (!s) return rows;
    return rows.filter(r => (r.zh || "").toLowerCase().includes(s) || (r.en || "").toLowerCase().includes(s));
  }, [rows, q]);

  return (
    <div style={{ padding: 16, maxWidth: 980, margin: "0 auto", fontFamily: "ui-sans-serif, system-ui" }}>
      <h1 style={{ fontSize: 22, fontWeight: 800 }}>Factory Glossary Admin</h1>

      <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 12 }}>
        <input
          value={pin}
          onChange={(e) => { _setPin(e.target.value); setPin(e.target.value); }}
        ...
        <button
          onClick={async () => {
            if (!confirm("確定清空整個 Factory Glossary？")) return;
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 8 }}>
        <input value={zh} onChange={(e) => setZh(e.target.value)} placeholder="zh" style={{ padding: 8 }} />
        <input value={en} onChange={(e) => setEn(e.target.value)} placeholder="en" style={{ padding: 8 }} />
        <input value={tags} onChange={(e) => setTags(e.target.value)} placeholder="tags (comma)" style={{ padding: 8 }} />
        <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="note (optional)" style={{ padding: 8 }} />
      </div>
      ...
      <textarea
        ...
        placeholder='[{"zh":"50模組","en":"50 module","tags":["factory"]}]'
      ...
      <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="search zh/en" style={{ padding: 8, width: "100%", marginTop: 8 }} />

      <div style={{ marginTop: 12, border: "1px solid #ddd", borderRadius: 8, overflow: "hidden" }}>
        <div style={{ display: "grid", gridTemplateColumns: "220px 1fr 220px 1fr", background: "#f6f6f6", padding: 10, fontWeight: 700 }}>
          <div>zh</div><div>en</div><div>tags</div><div>note</div>
        </div>
        {filtered.map((r) => (
          <div key={r.zh} style={{ display: "grid", gridTemplateColumns: "220px 1fr 220px 1fr", padding: 10, borderTop: "1px solid #eee" }}>
