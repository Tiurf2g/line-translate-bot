...
def reply_line(reply_token: str, text: str):
    ...
    requests.post(LINE_REPLY_API, headers=headers, json=payload, timeout=10)
...
@app.get("/")
def root():
    return {"ok": True, "msg": "LINE webhook alive"}
...
async def webhook(request: Request):
    ...
    # 驗簽失敗也回 200，避免 LINE 一直報錯
    if not verify_line_signature(body, signature):
        print("⚠️ signature invalid (ignored)")
